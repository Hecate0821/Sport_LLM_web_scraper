...
def get_andscape(page_num):
    url = article_link_list[page_num]

    response = requests.get(url)
    try:
        soup = BeautifulSoup(response.text, 'html.parser')
        art = soup.find('div',class_="story-body")
        lines = art.text.strip().split('\n')
        if 'Up Next From Sports' in lines[0]:
            lines = [line for line in lines if line.strip() != '' and 'http' not in line and 'AP' not in line and 'TwitterFacebookEmail' not in line and 'Up Next From Sports' not in line and '@' not in line]
            paragraph = '\n'.join(lines)
            return paragraph
        else:
            return 'error'
