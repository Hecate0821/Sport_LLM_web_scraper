...
def get_sportingnews(page_num):
    ...
    try:
        ...
        art = soup.find('div', class_='article-page')
        if art == None:
            return 'Error'
        else:
            lines = art.text.strip().split('\n')
            lines = [line for line in lines if line.strip() != '' and 'sporting News' not in line and 'min read' not in line and 'Sporting News' not in line and 'Associated Press' not in line]
            paragraph = '\n'.join(lines)
            return paragraph
    except:
        return 'Error'
